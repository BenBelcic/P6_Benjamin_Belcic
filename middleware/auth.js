...
module.exports = (req, res, next) => {
    try {
        ...
        const userId = decodedToken.userId;                               // on récupère dans le token décodé de la propriété userId
        req.auth = {                                                      // on ajoute la valeur décodée à l'objet request qui sera transmis aux routes
            userId: userId
        };
        next();
    } catch (error) {
